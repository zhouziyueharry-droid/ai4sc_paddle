...
def test_FNOBlock_output_scaling_factor():
    ...
    mlp_skip = 'linear'
    for dim in [1, 2, 3]:
        block = FNOBlocks(3, 4, max_n_modes[:dim], max_n_modes=max_n_modes[
            :dim], n_layers=1)
        ...
        assert block.convs.n_modes[:-1] == max_n_modes[:dim - 1]
        assert block.convs.n_modes[-1] == max_n_modes[dim - 1] // 2 + 1
        block = FNOBlocks(3, 4, n_modes[:dim], n_layers=1,
            output_scaling_factor=0.5, use_mlp=True, mlp_dropout=
            mlp_dropout, mlp_expansion=mlp_expansion, mlp_skip=mlp_skip)
        x = paddle.randn(shape=[2, 3, *size[:dim]])
        res = block(x)
        assert list(tuple(res.shape)[2:]) == [(m // 2) for m in size[:dim]]
        block = FNOBlocks(3, 4, n_modes[:dim], n_layers=1,
            output_scaling_factor=2, use_mlp=True, mlp_dropout=mlp_dropout,
...
def test_FNOBlock_norm(norm):
    ...
    dim = 2
    ada_in_features = 4
    block = FNOBlocks(3, 4, modes[:dim], n_layers=1, use_mlp=True, norm=
        norm, ada_in_features=ada_in_features, mlp_dropout=mlp_dropout,
        mlp_expansion=mlp_expansion, mlp_skip=mlp_skip)
    if norm == 'ada_in':
        embedding = paddle.randn(shape=ada_in_features)
        block.set_ada_in_embeddings(embedding)
    x = paddle.randn(shape=[2, 3, *size[:dim]])
    res = block(x)
    assert list(tuple(res.shape)[2:]) == size[:dim]
