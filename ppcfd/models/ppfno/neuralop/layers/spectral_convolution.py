...
def get_contract_fun(weight, implementation="reconstructed", separable=False):
    ...
    if implementation == "reconstructed":
        ...
        #         return _contract_cp
            ...
            raise ValueError(
                f"Got unexpected factorized weight type {type(weight).__name__}"
            )
    else:
        raise ValueError(
...
class FactorizedTensor(nn.Layer):
    ...
    def normal_(self, mean=0.0, std=1.0):
        ...
        This mirrors the API expected by the spectral convolution module.
        Initializes both real and imaginary parts with N(mean, std).
        """
        if mean != 0.0:
            raise ValueError(f"Currently only mean=0 is supported, but got mean={mean}")
        with paddle.no_grad():
            # Use set_value with paddle.randn to avoid relying on .normal_
            self.real.set_value(paddle.randn(self.real.shape) * std)
            self.imag.set_value(paddle.randn(self.imag.shape) * std)
        return self

    class _ComplexWeightSlice:
        ...
        def __getitem__(self, idx):
            if idx == 0:
                ...
                raise IndexError("Complex weight slice supports indices 0 (real) and 1 (imag)")

    def __getitem__(self, indices):
        """Return an object carrying real/imag slices and shape."""
        if isinstance(indices, list):
            indices = tuple(indices)
        real_slice = self.real[indices]
        imag_slice = self.imag[indices]
        return self._ComplexWeightSlice(real_slice, imag_slice)
...
class SpectralConv(BaseSpectralConv):
    ...
    def __init__(
        ...
        if joint_factorization:
            # Initialize joint weight with a Normal initializer to avoid relying on `.normal_` API
            ...
            # Initialize layer-wise weights; prefer tensor's own normal_ if provided,
            # otherwise fall back to initializing known parameter attributes.
...
class FactorizedSpectralConv1d(SpectralConv):
...
class FactorizedSpectralConv2d(SpectralConv):
    def forward(self, x, indices=0):
